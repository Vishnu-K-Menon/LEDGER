...
- **Tables are split by rows and never mixed with prose — but the header switches are inert today.** `HybridChunker` does split a table larger than `max_tokens` into row slices, and `merge_peers: false` does keep a slice from merging with prose (both pinned by config validators and a T3 test). `repeat_table_header: true` and `omit_header_on_overflow: false` are pinned but **have no effect under the default `TripletTableSerializer`**, which emits one `". "`-joined line per slice and supplies no header line at all: `get_header_and_body_lines` returns `[]` in the base serializer and is overridden only by the HTML and markdown serializers (VERIFIED 2026-09-26 against the installed docling-core 2.97.1; file:line evidence in D-033's status line). Each cell still carries its own column header inside the triplet, which is what keeps D-005 chunk-ID citation sound; what a slice does **not** carry is the table's unit/period statement, because Docling attaches no caption to any table. Consequence of the same code path: the headings prefix is added on top of a full-budget slice, so **78.7 % of table slices exceed `max_tokens` 512** (max 557), always by at most the headings token count. `chunk_type` is `table` iff the chunk holds **≥ 1 table item and every other doc item is a caption or footnote** — Docling emits the caption as its own item, so the stricter reading counted every unsplit table as prose (D-036); measured effect on the pilot is zero, the clause cannot fire while `merge_peers` is false. `max_tokens` is the main lever on A6 difficulty (more slices = more near-identical siblings); v1 holds 512. **Pending D-037 (markdown table serializer + one pre-freeze re-chunk, next pass after the A1 verdict) — not in force; do not switch the serializer before that entry is logged.** (D3 as revised by D-033 and D-036)
