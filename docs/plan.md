...
- [ ] **T3 · 4.5 h attention (was 3 h; re-baselined under D-032/D-033)** Docling parse (TableFormer accurate) **+ HybridChunker at ingest** (row-split tables, header on every slice, headings/captions via `contextualize`, 512 tok; CPU, tokenizer only) on the first 20 documents; pin `docling` and `docling-core` in `pyproject.toml`; tests: the three chunker switches read back from the constructed chunker, header kept on an overflowing row, headings/captions present on every slice, no chunk mixes table and prose items. **Wall-clock is unbounded until measured:** on the first 5 documents measure **pages/second**, single-process and with a process pool (Docling is stateless across processes), and report the extrapolation to the full corpus **per page** before parsing the remainder — a large figure is the owner's decision, not a reason to cut the corpus. Then the rest after A9.
...
- [ ] **T4 · 3 h** Embed the ingested chunks with Qwen3-Embedding-4B bf16; Qdrant local file via the single `make_client` (D-002); **freeze chunk IDs** (`data/chunk_ids.lock`) — IDs were assigned at ingest (D-032); nothing re-chunks between T3 and here.
...
- [ ] **T14 · 1 h** Matrix runner: `experiments/matrix.yaml` → Batch jobs, one seed at a time (D-009) → `results/<cell>/<seed>.jsonl`. **Resumable across sittings (D-025):** submit → persist batch ids → exit; a later invocation polls and collects. **Results and `reports/smoke.json` keep per-question unsupported share, per-claim verdicts and per-question gold-in-context — never aggregates (D-030); the smoke gate is computed from these.**
...
- [ ] **T17 · 4 h** README with the numbers and caveats (fixed arms; 512 convention; OpenInference-not-gen_ai with the Aug 2026 date; judge/question-writer family note); CLI or notebook demo; `docs/decisions.md` updated; `ledger smoke` on demand — gate per D-030 (structural assertions fail outright; paired t + bootstrap on the unsupported share; exact McNemar on `evidence_recall@STOP`; 90 % two-sided; worse → fail, better → flag), centred on seed 1's per-question smoke results; `reports/smoke.json` stores per-question share, per-claim verdicts, per-question gold-in-context — not aggregates; run after any post-seed-1 code change and once before the README numbers, **never in CI** and not before each seed (D-028, D-029; CI = `pytest` + `ruff`).
