...
**Status 2026-09-20 (units).** The stop counts **units**, not documents: `ingest.confirm_after_units: 20` (renamed from `confirm_after_docs`); one manifest row = one unit, granule or report (D-034). The parse-stage guard reads `unit_count(manifest)`. Body above unedited.
...
**Status 2026-09-20 (listing pass run, seed 20260920).** (1) **EIA frame actually used:** `sitemap.xml` is a flat list of 133 HTML pages with **no PDFs**; the frame is sitemap → the three landing pages (`totalenergy/data/monthly/`, `outlooks/steo/`, `outlooks/aeo/`) → PDF links on them; it yields 13 MER section PDFs + STEO full + AEO narrative = 15 units. (2) **Granule `pages` is absent** from every GovInfo granule summary; fallback at listing = package `pages` ÷ granule count, recorded as `pages_source: estimated`; **at `--stage fetch` `pages` is measured from the file with pypdf and `pages_source` flips to `measured`**. (3) **BUDGET granules carry no `pdfLink` and no `txtLink`** (tables expose `xlsLink` only) → gate check 1 fails → BUDGET is **report-level**; the package PDFs are the units. (4) **ERP table granules' `txtLink` (`/htm`) rendition is a ~350-byte stub**, so checks 2 and 3 are **field-absent**, not passed; ERP is granule-level by the fail-on-any-check rule, with self-containment **still unverified — it is checked on the fetched PDFs before parse**. (5) One-edition-per-series keys on the package-id series code (`BUDGET-2027-APP` → `BUDGET-APP`, `ERP-2026` → `ERP`), not the title, which changes between editions. (6) `api.govinfo.gov/robots.txt` returns 500; the fetcher treats 401/403 as never-fetch and 404/5xx as no-rules-allow, logged. (7) CRPT filter base rate: 30/40 sampled packages contain a CBO cost estimate. Body above unedited.
**Status 2026-09-20 (owner decisions on the draw — confirmations, not new decisions).** (1) The 20-unit draw in `data/manifest.jsonl` (seed 20260920, snapshot 2026-09-20) is **confirmed**. (2) `govinfo_erp` **stays granule-level**; self-containment is verified on the two fetched granule PDFs (title present · own header row on page 1 · no carried-over table on page 1 · no run-off on the last page); any FAIL demotes ERP to report-level (1 unit, pilot = 19) with **no replacement draw** — a redraw on a failed check is selection on outcome. (3) `BUDGET-2027-TAB` (21 pp, metadata) **stays in the draw**; if intro-only that is a fact about the source recorded for A9, not a pilot failure. Body above unedited.
**Status 2026-09-20 (fetch stage run).** `ledger ingest --stage fetch --draw-confirmed`: 19 of 20 units on disk under `data/raw/<source>/`; sha256, measured pages (pypdf; `pages_source: measured`), `text_layer_ratio`, policy fields, and EIA `date_issued` (from PDF CreationDate) filled; manifest rewritten in place; a re-run reused 18 files by hash. **Findings:** (a) `BUDGET-2027-TAB` (Historical Tables) has **no PDF at all** — package and granules expose `premis`/`zip`/`mods`/`xls` only; the row stays in the manifest marked UNFETCHABLE (owner decision 3 covers intro-only; this is stronger — the owner decides its fate at A9; the parse stage skips it). (b) `CRPT-118hrpt468` has no package-level `pdfLink`; the PDF lives on its single `FIRSTPART` granule and was resolved through it (recorded on the row; no redraw). (c) **ERP self-containment: both granules pass all four checks**; the header check found the unit statement on page 1 (`[Percent of nominal GDP]`, `[Monthly data seasonally adjusted, except as noted]`) — the D1 premise is on the granule. **ERP stays granule-level.** (d) ERP granule pages: listing estimate 6 (450 ÷ 80) → **measured 2** each. (e) Below `text_layer_ratio_min` 0.95, kept and flagged: `BUDGET-2027-FCS` 0.917, `BUDGET-2026-CROSSCUT` 0.918 (blank/graphic pages, not scans — to confirm at A1). (f) pypdf renders the decimal separator in ERP tables as U+FFFD (`–2�5` = −2.5): a text-layer font-encoding artifact that A1 must check Docling against — if Docling shows the same, the ERP text layer does not carry the digits D2 assumes. (g) HTTP calls: eia.gov 8, api.govinfo.gov 6 (+4 on the re-run); cbo.gov 0. Body above unedited.
