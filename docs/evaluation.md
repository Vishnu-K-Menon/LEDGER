...
100 questions, stratified table/prose. For each: one baseline claim chosen at random from the no-repair answer; the paired post-repair claim from the `re_retrieve(1) × claimify` cell, seed 1 — the same `claim_id` if it survived unchanged, the claim recorded in `replaces` if it was repaired, a random claim from the post-repair answer if it was deleted (pairing rule — owner to confirm, see D-010). Three-class labels. `eval/labels.py` refuses a set whose halves do not share the same 100 question ids. Analysis: per-question unsupported indicator difference, paired bootstrap.
...
Unit tests on every push: schemas, ledger invariants (a SUPPORTED claim cannot have empty `evidence_ids`; `iter_resolved ≥ iter_first_seen`), stop rule, config loading, the D-002 single-client guard, the D-006 no-retrieval guard, the D-008 marker-stripping identity, a test that the matrix runner never calls the generator synchronously (Batch only), a test that no module under `tracing/` or `eval/` imports `phoenix`, and a test that every post-hoc evaluation carrier has exactly one Span Link (D-017). On demand: `ledger smoke` — 20 questions (~$0.50) writing `reports/smoke.json`; fails if unsupported rate or `evidence_recall@STOP` moves beyond a tolerance set from the week-4 run. Smoke eval is cut #5 if week 3 overruns.
