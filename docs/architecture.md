...
**Pins:** `openinference-semantic-conventions==0.1.29`, `openinference-instrumentation>=0.1.57` (D-017; confirm on PyPI at T1).
**Instance rejected (D-015):** t3.small with swap + gp3 + memory alarm — no headroom for bursty Batch-result ingest plus the week-4 UI.
